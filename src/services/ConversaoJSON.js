...
function convertDetalheLine(line) {
    var detalhe = {
...
function convertCnab750ToJson(cnab750) {
    var lines = cnab750.split('\n');
    var header = lines[0];
    var detalhes = lines.slice(1, lines.length - 3);
    var registroDetalheInformacoesAdicionais = lines[lines.length - 3];
    var dadosEspecificosDeCobrancaComVencimento = lines[lines.length - 2];
    var trailer = lines[lines.length - 1];
    var data = {
        HEADER: {
...
var cnab750 = fs.readFileSync('arquivo.cnab', 'utf-8');
// Converter o arquivo CNAB 750 em um objeto JSON
var jsonData = convertCnab750ToJson(cnab750);
